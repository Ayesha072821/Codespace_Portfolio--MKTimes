...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sort_option'])) {
    ...
    if ($sort_option === 'low_to_high') {
        ...
        usort($mdata, fn($a, $b) => $a['item_price'] <=> $b['item_price']);
    } elseif ($sort_option === 'high_to_low') {
        // Sort by price: high to low
        usort($mdata, fn($a, $b) => $b['item_price'] <=> $a['item_price']);
    }

    elseif ($sort_option === 'A_Z') {
        // Sort by price: high to low
        usort($mdata, fn($a, $b) => $a['item_name'] <=> $b['item_name']);
    }

    elseif ($sort_option === 'Z_A') {
        // Sort by price: high to low
        usort($mdata, fn($a, $b) => $b['item_name'] <=> $a['item_name']);
    }
...
<form method="post" class="text-center my-4">
    ...
    <?php
        // displaying all data with 3 cards in a row
        $counter = 0;
